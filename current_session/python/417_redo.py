...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        # use dfs
        # use 2d array to store the visit status
        # pac: 1=reachable, 0=not reachable, -1=unvisited
        H, W = len(heights), len(heights[0])
        pac = [[-1 for _ in range(W)] for _ in range(H)]
        atl = [[-1 for _ in range(W)] for _ in range(H)]

        def dfs(i, j, ocean):
            ocean[i][j] = 1
            for a, b in [(i+1,j),(i-1,j),(i,j+1),(i,j-1)]:
                if 0<=a<H and 0<=b<W and ocean[a][b] == -1:
