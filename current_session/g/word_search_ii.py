...
class Solution:
    def findWords(self, board, words):
        ...
        return [x for x in self.onboard]
    
    def dfs(self, i, j, node, prefix):
        if node.isWord:
            self.onboard.add(prefix)
            node.isWord = False
        ...
        for a, b in [(i,j+1), (i,j-1), (i+1,j), (i-1,j)]:
            self.dfs(a,b,node.children[char],prefix+char)
        self.B[i][j] = char
        return
