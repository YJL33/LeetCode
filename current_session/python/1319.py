...
class Solution:
    def makeConnected(self, n: int, connections: List[List[int]]) -> int:
        # the most efficient way of connecting all computers is using n-1 wires
        if len(connections) < n-1: return -1
        
        # count isolated group of computers (say x groups)
        # return x-1
        ...
        for a, b in connections:
            ...
            rb = get_who(b)
            if ra != rb:
                who[max(ra,rb)] = min(ra, rb)
        
        return sum([who[i] == i for i in range(n)])-1
