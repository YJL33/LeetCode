...
class Solution(object):
    def findRightInterval(self, intervals):
        ...
        """
        # TLE
        # construct a dict which records the interval, key = start, val = original index
        # construct another sorted list to store all starts
        # for every element in sorted list, output its min right counterpart
        # for i in intervals: print i.start, i.end,
        intdct, starts, ends = {}, [], []
        
        for i in xrange(len(intervals)):
            starts += intervals[i].start,
            ends += (intervals[i].end, i),
            intdct[intervals[i].start] = i

        starts.sort()
        ends.sort(reverse=True)
        # print starts
        ans = [-1 for _ in xrange(len(starts))]
        istart = len(starts)-1
        for i in xrange(len(ends)):
            while istart-1 >= 0 and starts[istart-1] >= ends[i][0]:
                istart -= 1
            if starts[istart] >= ends[i][0]:
                ans[ends[i][1]] = intdct[starts[istart]]
